...
class FakePlanRepository : PlanRepository {
    private val mutablePlan = MutableStateFlow<List<PlannedMeal>>(emptyList())
    val assignedMeals = mutableListOf<Triple<Long, Int, Int>>()
    val clearedSlots = mutableListOf<Pair<Int, Int>>()

    fun setPlan(meals: List<PlannedMeal>) {
        mutablePlan.value = meals
    }

    override fun getWeekPlan(weekStartEpochDay: Long): Flow<List<PlannedMeal>> = mutablePlan

    override suspend fun assignMeal(
